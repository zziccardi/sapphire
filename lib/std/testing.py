...
def assert_true(cond: bool, msg: str = ""):
  ...
  if not cond:
    ctx.record_failure(_format_msg("Expected condition to be true", msg),
                       fatal=True, caller_frame=inspect.currentframe().f_back)
  else:
    ctx.passed_assertions += 1

def assert_false(cond: bool, msg: str = ""):
  ctx = get_active_context()
  if cond:
    ctx.record_failure(_format_msg("Expected condition to be false", msg), fatal=True, caller_frame=inspect.currentframe().f_back)
  else:
    ctx.passed_assertions += 1
...
def assert_ne(actual, expected, msg: str = ""):
  ctx = get_active_context()
  if actual == expected:
    ctx.record_failure(
...
def assert_not_none(opt, msg: str = ""):
  ctx = get_active_context()
  if opt is None:
    ctx.record_failure(
...
def expect_true(cond: bool, msg: str = ""):
  ctx = get_active_context()
  if not cond:
    ctx.record_failure(_format_msg("Expected condition to be true", msg), fatal=False, caller_frame=inspect.currentframe().f_back)
  else:
    ctx.passed_assertions += 1

def expect_false(cond: bool, msg: str = ""):
  ctx = get_active_context()
  if cond:
    ctx.record_failure(_format_msg("Expected condition to be false", msg), fatal=False, caller_frame=inspect.currentframe().f_back)
  else:
    ctx.passed_assertions += 1

def expect_eq(actual, expected, msg: str = ""):
  ctx = get_active_context()
  if actual != expected:
    ctx.record_failure(
        _format_msg(f"Expected {expected!r}, got {actual!r}", msg), fatal=False, caller_frame=inspect.currentframe().f_back)
  else:
    ctx.passed_assertions += 1

def expect_ne(actual, expected, msg: str = ""):
  ctx = get_active_context()
  if actual == expected:
    ctx.record_failure(
        _format_msg(f"Expected value to not equal {expected!r}", msg),
...
def expect_not_none(opt, msg: str = ""):
  ctx = get_active_context()
  if opt is None:
    ctx.record_failure(
        _format_msg("Expected non-None value, got None", msg),
